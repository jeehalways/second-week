...
*/
const addTwo = (number: number): number => number + 2;
...
*/
const addS = (word: string): string => word + "s";
...
*/
const map = <T, R>(array: T[], callback: (el: T) => R): R[] => {
  const result: R[] = [];
...
const forEach = <T>(
  array: T[],
  callback: (el: T, index: number, arr: T[]) => void
): void => {
  let i = 0;
  for (const el of array) {
    callback(el, i, array);
...
*/
const mapWith = <T, R>(array: T[], callback: (el: T) => R): R[] => {
  const result: R[] = [];
